...
import { findJobByOrder } from "../jobs/store.js";

const log = logger("hosting");

/**
 * The hosting loop: `aacp-watch.mjs wait` blocks until a buyer message / funded order
 * appears (its polling is what keeps the agent ONLINE), then events are dispatched.
 * Card builds run in a bounded worker pool so chat stays responsive while Blender renders.
 */
export class HostingLoop {
  private stopped = false;
  private running = new Map<string, Promise<void>>();
  private queue: string[] = [];
  private redoNotes = new Map<string, string>();
  private lastSweep = 0;
  ...
  private enqueueOrder(orderId: string, reason: string, redoNote?: string) {
    if (this.running.has(orderId) || this.queue.includes(orderId)) return;
    log.info(`order ${orderId}: queued (${reason})`);
    if (redoNote) this.redoNotes.set(orderId, redoNote);
  ...
  private pump() {
    ...
    while (this.running.size < jobs.concurrency && this.queue.length) {
      const orderId = this.queue.shift()!;
      const redoNote = this.redoNotes.get(orderId);
      this.redoNotes.delete(orderId);
      const p = processOrder(orderId, { redoNote })
        .then(() => undefined)
  ...
  private async dispatch(ev: WatchEvent) {
    switch (ev.type) {
      ...
      case "hosting.handoff":
        ...
        break;
      case "offer.received":
        log.info("offer.received (we act as provider only) — ignored", { orderId: ev.orderId });
        break;
      default:
  ...
  private async sweep() {
    ...
    try {
      const ids = await sweepOrders();
      for (const id of ids) {
        const job = findJobByOrder(id);
        this.enqueueOrder(id, "sweep", job?.status === "delivered" ? "redo requested by buyer (details in the order payload)" : undefined);
      }
