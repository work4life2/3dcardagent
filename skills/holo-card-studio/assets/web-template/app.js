...
async function init(){
 ...
 document.title=config.title+' · 幻光典藏';for(const [id,key]of Object.entries({'card-title':'title','collection':'collection','subtitle':'subtitle','description':'description','tagline':'tagline','technique':'technique','edition':'edition'}))if(config[key])$(id).textContent=config[key]; fitTitle();addEventListener('resize',fitTitle);document.fonts?.ready.then(fitTitle);
...
// A long English title ("Autonomous AI Spirit") used to run past the story column and over the card
// stage. Shrink the display size until the title fits its column in at most two lines.
function fitTitle(){const h=$('card-title');if(!h)return;h.style.removeProperty('--title-size');let size=parseFloat(getComputedStyle(h).fontSize);if(!size)return;const limit=()=>size*1.14*2+2;h.style.setProperty('--title-size',size+'px');while(size>26&&(h.scrollWidth>h.clientWidth+1||h.getBoundingClientRect().height>limit())){size-=2;h.style.setProperty('--title-size',size+'px');}}
